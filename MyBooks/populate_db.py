...
def populate_db():
    ...
    with app.app_context():
        ...
        UserTrophy.query.delete()
        BookReading.query.delete()
        Trophy.query.delete()
        Book.query.delete()
        User.query.delete()
        ...
        teste_user = User.query.filter_by(username="teste").first()
        
        # Adiciona leituras de livros de Ficção Científica para o usuário teste
        for i in range(1, 4):  # Adiciona 3 livros de Ficção Científica
            reading = BookReading(user_id=teste_user.id, book_id=i)
            db.session.add(reading)
            book = Book.query.get(i)
            teste_user.points += book.pages // 100 + 1
        
        # Adiciona leituras de livros de Fantasia para o usuário teste
        for i in range(6, 8):  # Adiciona 2 livros de Fantasia
            reading = BookReading(user_id=teste_user.id, book_id=i)
            db.session.add(reading)
            book = Book.query.get(i)
            teste_user.points += book.pages // 100 + 1
